...
结论先说清楚：功能闭环通过。Python 单元测试 6/6 通过；硬判决 RTL 仿真 4 个必需 case 全 0 mismatch；软判决 RTL 仿真 3 个 AWGN case 全 0 mismatch；M7 最新板级 run_id 为 `m7_board_20260502_083547`，UART 显示 3 个 case 全部 0 mismatch。限制也必须先说清楚：M6 在 100 MHz 目标下 hero design 时序未收敛，布局布线后的 WNS 为 -20.433 ns；M7 board shell 降到 25 MHz 后通过，说明当前架构功能正确但还不是 100 MHz timing-clean design（时序干净设计：所有路径都能在目标时钟内到达的硬件版本）。
...
![Fig04 state explosion](docs/assets/plots/fig04_state_explosion.png)

**图 4：状态数随 K 指数增长。** 这张图来自 `scripts/build_final_report.py` 的公式 `2^(K-1)`，回答为什么 K=7 会直接变成 64 个状态。它不是实验结果图，而是设计复杂度图。

![Fig05 hard vs soft metric](docs/assets/plots/fig05_hard_vs_soft_metric.png)
...
![Fig09 normalization](docs/assets/plots/fig09_normalization_effect.png)
...
![Fig14 depth tradeoff](docs/assets/plots/fig14_depth_tradeoff.png)
...
![Fig15 width tradeoff](docs/assets/plots/fig15_width_resource_tradeoff.png)
...
![Fig16 fmax resource](docs/assets/plots/fig16_fmax_resource_plot.png)
...
**表：M7 UART case summary。** 数据来源 `data/board_runs/m7_board_20260502_083547/uart_capture.json` 和 `data/board_runs/board_summary.csv`。每个 case 都看到 204 input samples 和 96 emitted output samples，mismatch_count 为 0。

![Fig17 board summary](docs/assets/plots/fig17_board_run_summary.png)
